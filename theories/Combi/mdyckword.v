...
From mathcomp Require Import ssrbool ssrfun ssrnat eqtype fintype choice seq tuple.
...
Definition is_dyck_height h w :=
  (brace_bal h w) /\ forall n, brace_pos h (take n w).
Definition is_dyck w := is_dyck_height 0 w.

Definition prefixes w := [seq take n w | n <- iota 0 ((size w).+1)].
Definition dyck_prefix_height h w :=
  brace_bal h w && all (brace_pos h) (prefixes w).
...
Notation "dyck_rec( h )" := (dyck_height_rec h)
                              (at level 2, format "dyck_rec( h )").
...
apply (iffP allP) => [H n| H n _ /=]; last by [].
case: (ltnP n (size w).+1) => Hn; first by apply H; rewrite mem_iota.
rewrite take_oversize; last by apply ltnW.
rewrite -(take_size w); apply H.
by rewrite mem_iota /=.
Qed.
...
- by case h1; case h2.
- by move=> H1 H2; apply/eqP; rewrite -(eqn_add2l m);apply/eqP/IHw.
- case: h1 => h1; case: h2 => h2 //= H1 H2.
  by apply/eqP; rewrite eqSS; apply/eqP; apply IHw.
Qed.

Lemma cat_dyck_height w1 w2 h1 h2 :
  w1 \in dyck_rec(h1) ->
  w2 \in dyck_rec(h2) ->
  (w1 ++ w2) \in dyck_rec(h1 + h2).
Proof.
elim: w1 w2 h1 h2 => [|[] w1 IHw1] w2 h1 h2 //=; first by case h1.
- by rewrite !open_dyck_height addnA; apply IHw1.
- by case: h1 => h1 //=; apply IHw1.
Qed.
...
exact: dyck_height_eq.
Qed.

Lemma dyck_prefix_heightP h w :
  reflect (is_dyck_height h w) (dyck_prefix_height h w).
Proof.
rewrite /is_dyck_height /dyck_prefix_height.
by apply: (iffP andP) => [] [H1 H2]; split => //; apply/positiveP.
Qed.

Lemma dyck_prefixP w : reflect (is_dyck w) (w \in dyck_prefix).
Proof. by rewrite /is_dyck /dyck_prefix; apply dyck_prefix_heightP. Qed.

Lemma dyckP w : reflect (is_dyck w) (w \in dyck_rec).
Proof. by rewrite -topredE /= -dyck_eq; apply dyck_prefixP. Qed.
...
- case: hw => hw; case: h => h //=.
  by move: IHw => /(_ hw h); elim (cut_to_height h w).
Qed.

Lemma cut_dyck_height w hw h :
  w \in dyck_rec(hw) -> hw > h ->
  let (w1, w2) := cut_to_height h w
  in w1 \in dyck_rec(h) /\ w2 \in dyck_rec(hw - S h).
Proof.
...
- case: h => h; first by rewrite subn1 //=.
  move: IHw => /(_ hw h Hd).
  by elim (cut_to_height h w) => w1 w2 IHw /=; apply IHw.
Qed.
...
- move: IHw => /(_ (m + hw) (m + h) w1 w2 Hd _ Hdw1) {Hd Hdw1}.
  ...
  by rewrite (ltn_add2l m) => IHw; apply (IHw Hp).
- by move: h Hp => [] Hp Hdw /= Happ //=; rewrite eq_sym.
- move: hw h Hp Hd Hdw1 => [] //= hw [] //= hw1 Hlt Hdw Hdw1.
  move: IHw => /(_ hw hw1 w1 w2 Hdw _ Hdw1) {Hdw Hdw1}.
  by elim (cut_to_height hw1 w) => wa1 wa2; apply.
Qed.
...
Lemma join_dyck_height l w hw :
  all (mem dyck_rec) l ->
  w \in dyck_rec(hw) ->
  join Close w l \in dyck_rec((size l) + hw).
Proof.
...
Variant mult_cut_spec n w l wr : Type :=
  MMultCutSpec:
    size l == n ->
     all (mem dyck_rec) l ->
     wr \in dyck_rec ->
...
Lemma mult_cutP n w :
  w \in dyck_rec(n) ->
  let (l, wr) := (mult_cut n w) in mult_cut_spec n w l wr.
Proof.
elim: n w => [| n IHn w H] /=.
  by rewrite -/dyck_rec.
have Hpos := ltn0Sn n.
...
move/IHn: Hdw2.
move: (mult_cut n w2) => [l wr] [Hlen Hld Hwr Hjoinl].
split => //=; first by rewrite Hld Hdw1.
...
Proof.
move=> []; elim: n w l wr => [| n IHn ] w l wr [Hlen] //.
...
by rewrite -(IHn (join _ _ _) l wr Hlen Halld Hwrd (eq_refl _)).
Qed.

Definition factor w := mult_cut m (behead w).

Record factorization w : Set :=
  Factorization { l; wr;
                  _ : (size l == m);
...
exact: (Factorization  (l:=l) (wr:=wr)).
Qed.

Lemma factored_is_dyck_non_nil w :
  factorization w -> w \in dyck_rec /\ (w != [::]).
Proof.
...
Proof. by elim: lD => [//=|D0 lD] /= <-. Qed.

Lemma seq_Dyck_all_dyck (lD : seq Dyck) : all (mem dyck_rec) (map dyckword lD).
Proof.
elim: lD => [//=| D0 ld IHlD].
by rewrite all_map; simpl; rewrite -all_map IHlD /= andbT; apply is_dyck_Dyck.
Qed.
...
Record Dyck_factorization D : Set :=
...
rewrite (eqP (seq_Dyck_val Halld)) (eqP Hszl) => /eqP HszD.
...
by have -> : ((factored_is_dyck fct) = Hw) by apply bool_irrelevance.
Qed.
...
rewrite (eqP Hsz) eq_sym => Halld /esym/eqP Hszl.
...
by rewrite size_cat /= addnS ?ltnS; apply leq_addl.
Qed.

Variable P : Dyck -> Type.
Hypothesis (Pnil : P emptyDyck)
           (Pcons : forall D (fct : Dyck_factorization D),
               (forall ww, ww \in val (lD fct) -> P ww) -> P (Dr fct) -> P D).
...
by rewrite /Dyck_lt_size => x y; apply /ltP.
Qed.

Definition Dyck_gram_ind : forall D, P D :=
  Fix RwfDyck (fun D =>  P D) Dyck_ind_rec.
